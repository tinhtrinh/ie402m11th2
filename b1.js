const b1base = {
  type: 'polygon',
  rings: [
    [106.78290256961101, 10.882587609855685, 0],
    [106.78291933341626, 10.882603413690127, 0],
    [106.7829079340287, 10.882619217523741, 0],
    [106.78303064508312, 10.882717991464759, 0],
    [106.78304606778394, 10.882704821607847, 0],
    [106.78306953711129, 10.882725893378622, 0],
    [106.78319023650906, 10.882581683417564, 0],
    [106.78316475552509, 10.88255863615704, 0],
    [106.78317563702103, 10.882544222869857, 0],
    [106.78305426707104, 10.882438863936837, 0],
    [106.78304085602683, 10.88245532627257, 0],
    [106.78302342166938, 10.882442156404053, 0],
  ],
};

const b1roof = {
  type: 'polygon',
  rings: [
    [106.78290256961101, 10.882587609855685, 45],
    [106.78291933341626, 10.882603413690127, 45],
    [106.7829079340287, 10.882619217523741, 45],
    [106.78303064508312, 10.882717991464759, 45],
    [106.78304606778394, 10.882704821607847, 45],
    [106.78306953711129, 10.882725893378622, 45],
    [106.78319023650906, 10.882581683417564, 45],
    [106.78316475552509, 10.88255863615704, 45],
    [106.78317563702103, 10.882544222869857, 45],
    [106.78305426707104, 10.882438863936837, 45],
    [106.78304085602683, 10.88245532627257, 45],
    [106.78302342166938, 10.882442156404053, 45],
  ],
};

const b1floor = [];

for (let i = 0; i <= 11; i++) {
  let r1, r2, r3, r4;

  if (i < 11) {
    r1 = b1base.rings[i];
    r2 = b1base.rings[i + 1];
    r3 = [
      b1base.rings[i + 1][0],
      b1base.rings[i + 1][1] + 0.00000000000001,
      10,
    ];
    r4 = [b1base.rings[i][0], b1base.rings[i][1] + 0.00000000000001, 10];
  } else {
    r1 = b1base.rings[11];
    r2 = b1base.rings[0];
    r3 = [b1base.rings[0][0], b1base.rings[0][1] + 0.00000000000001, 10];
    r4 = [b1base.rings[11][0], b1base.rings[11][1] + 0.00000000000001, 10];
  }

  let f = {
    type: 'polygon',
    rings: [r1, r2, r3, r4],
  };

  b1floor.push(f);
}

const b1wall = [];

for (let i = 0; i <= 11; i++) {
  let w1, w2, w3, w4;

  if (i < 11) {
    w1 = [b1base.rings[i][0], b1base.rings[i][1], 10];
    w2 = [b1base.rings[i + 1][0], b1base.rings[i + 1][1], 10];
    w3 = [
      b1base.rings[i + 1][0],
      b1base.rings[i + 1][1] + 0.00000000000001,
      45,
    ];
    w4 = [b1base.rings[i][0], b1base.rings[i][1] + 0.00000000000001, 45];
  } else {
    w1 = [b1base.rings[11][0], b1base.rings[11][1], 10];
    w2 = [b1base.rings[0][0], b1base.rings[0][1], 10];
    w3 = [b1base.rings[0][0], b1base.rings[0][1] + 0.00000000000001, 45];
    w4 = [b1base.rings[11][0], b1base.rings[11][1] + 0.00000000000001, 45];
  }

  let w = {
    type: 'polygon',
    rings: [w1, w2, w3, w4],
  };

  b1wall.push(w);
}

export { b1base, b1roof, b1floor, b1wall };
